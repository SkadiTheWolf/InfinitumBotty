...
class DiceObserver(PrivMsgObserverPrototype):
    ...
    def update_on_priv_msg(self, data: dict, connection: Connection):
                ...
                if dice_sides.isdigit():
                    ...
                    if dice_sides == 0:
                        connection.send_back(
                            f"{data['nick']} wirft einen 0-seitigen Würfel und bekommt Unendlich.",
                            data,
                        )
                        return
                elif dice_sides[0] == "-":
                    ...
                    connection.send_back(
                        f"{data['nick']} wirft einen {str(dice_sides)}-seitigen Würfel und nichts passiert.",
                        data,
                    )
                    return
                else:
        ...
        connection.send_back(
            f"{data['nick']} wirft einen {str(dice_sides)}-seitigen Würfel und bekommt {str(result)}",
            data,
        )
