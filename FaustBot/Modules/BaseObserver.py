...
def small_to_big(von, zu, zahl):
    out = 'not implemented yet'

    return out

def big_to_small(von, zu, zahl):
    out = 'not implemented yet'
...
class BaseObserver(PrivMsgObserverPrototype):
    ...
    def update_on_priv_msg(self, data: dict, connection: Connection):
        ...
        if data['message'].startswith(".base"):
            ...
            array = data['messageCaseSensitive'].split(' ', 3)
            ...
            if zu == 10:
                ...
                out = big_to_small(von, zu, zahl)

            elif not bigger:
                out = small_to_big(von, zu, zahl)
...
'''
            match base:
                case '2':
                    try:
                        zahlDezimal = int(zahlString)
                        output = '{0:b}'.format(zahlDezimal)
                        connection.send_back(f'Die Zahl {zahlString} entspricht {output}', data)
                    except ValueError:
                        connection.send_back('Fehler: Keine Ganzzahl', data)

                case '10':
                    try:
                        output = int(zahlString, 2)
                        connection.send_back(f'Die Zahl {zahlString} entspricht {output}', data)
                    except ValueError:
                        connection.send_back('Fehler: Keine Zahl', data)

                case _:
                    connection.send_back(f'Base {base} nicht verfügbar', data)
'''