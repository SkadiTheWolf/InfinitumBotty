    ...
    def add(self, data, connection):
        if self.gamestatus == 0:
            connection.send_channel(
                "Es läuft derzeit kein Wordrun, bitte einen neuen mit .begin <Silbe> oder .end <Silbe> erstellen!"
            )
            return
        ...
        for p in self.player.keys():
            for w in self.player[p]:
                if w.upper() == word.upper():
                    connection.send_channel(
                        "Das Wort " + word + " wurde bereits von " + p + " genannt"
                    )
                    return
    ...
    def rules(self, data, connection):
        if data["channel"] == connection.details.get_channel():
            connection.send_back(
                "Spielregeln bitte im Query (Privatchat) mit dem Bot abfragen", data
            )
            return
        connection.send_back(
            "Wordrun Spiel: So viele Wörter wie möglich finden, die mit der vorgegebenen Silbe anfangen oder aufhören.",
            data,
        )
        connection.send_back(
            "Spiel starten mit .begin <Silbe>, um ein Spiel zu starten, bei dem die Antworten mit <Silbe> anfangen müssen.",
            data,
        )
        connection.send_back(
            "Spiel starten mit .end <Silbe>, um ein Spiel zu starten, bei dem die Antworten mit <Silbe> enden müssen.",
            data,
        )
        connection.send_back(
            "Antwort hinzufügen mit .a <Antwort> oder .add <Antwort>", data
        )
        connection.send_back(
            "Es können auch mehrere Antworten in einer Zeile angegeben werden", data
        )
        connection.send_back(
            "Das Spiel dauert 3 Minuten. Für jede gültige Antwort gibt es 1 Punkt.",
            data,
        )
