...
class GlossaryModule(PrivMsgObserverPrototype):
    ...
    def cmd():
        return [
            GlossaryModule._ADD_EXPLANATION,
    ...
    def _answer_query(self, data, connection: Connection):
        ...
        if answer is None or answer[1] is None or answer[1].strip() == "":
            if split[1].strip() == "":
                ...
                #     f"Tut mir leid, {data['nick']} . Für {split[1].strip()} habe ich noch keinen Eintrag. Aber Wikipedia sagt dazu:",
                #     data,
                # )
            ...
            connection.send_back(f"{data['nick']}: {split[1]} - {answer[1]}", data)
    ...
    def _remove_query(self, data, connection: Connection):
        ...
        if not self._is_idented_mod(data, connection):
            connection.send_back(
                f"Dir fehlen die Berechtigungen zum Löschen von Einträgen, {data['nick']}.",
                data,
            )
            return
        glossary_provider = GlossaryProvider()
        ...
        connection.send_back(
            f"Der Eintrag zu {split[1]} wurde gelöscht, {data['nick']}.",
            data,
        )
    ...
    def _add_query(self, data, connection: Connection):
        ...
        if not self._is_idented_mod(data, connection):
            connection.send_back(
                f"Dir fehlen leider die Rechte zum Hinzufügen von Einträgen, {data['nick']}.",
                data,
            )
            return
        msg = (
        ...
        connection.send_back(
            f"{data['nick']}: der Eintrag zu {split[0]} wurde gespeichert.", data
        )
